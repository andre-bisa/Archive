...
public class AgendaController {
	...
	public AgendaController(ViewFactory viewFactory, ContactsPersister persister) {
		this.viewFactory = viewFactory;
		this.manager = new PersistenceManager(persister);
	}
	...
	public boolean loadContacts(String filename) {
		try {
			...
			return false;
		} catch (agenda.exceptions.BadFileFormatException e) {
			viewFactory.createMessageDialog().showMessage(
	...
	public boolean deleteContact(Contact contact) {
		if (contact != null) {
			...
			if (result == DialogResult.Yes) {
				manager.getAgenda().getContacts().remove(contact);
				return true;
			}
		}
		return false;
	...
	public boolean insertContact() {
		...
		if (contact != null) {
			manager.getAgenda().getContacts().add(contact);
			return true;
		}
		return false;
	}
