...
RESULT compareTo3(TYPE v1[], TYPE v2[], int dim1, int dim2) {
	...
	int checked[DIM]; //array di appoggio per vedere se l'elemento dell'array di sopra è già stato usato

	//svuoto il vett di appoggio
	for (i = 0; i < DIM; i++)
		checked[i] = 0;
	...
	if (dim1 != dim2) {
		...
		for (i = 0; i < dim1 && ritorno == EQUAL; i++) {
			if (!checked[i]) { // non ancora utilizzato
